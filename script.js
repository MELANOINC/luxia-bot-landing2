...
import { N8N_BASE_URL, ENDPOINTS, apiUrl, ASSISTANT, CONTACT, generateWhatsAppUrl } from './config.js';
import { analyzeLeadWithQuantumAI } from './lib/quantum-ai.js';
import MelaniaAssistant from './components/MelaniaAssistant.js';
...
/**
 * Melania Assistant Widget Handler
 */
function initAssistantWidget() {
  try {
    // Expose minimal config for the widget to use
    window.AppConfig = { CONTACT, generateWhatsAppUrl };
    const assistant = new MelaniaAssistant(ASSISTANT);
    assistant.init();
    window.Melania = assistant;
  } catch (e) {
